...
#length(GENERAL_SKILLS)
    ...
    #for (j in seq_along(PERSONALITY_SKILLS)) {
    #jd_df$pers<- ifelse(grepl(PERSONALITY_SKILLS[j],jd_df$job_descr[i]),as.character(PERSONALITY_SKILLS[j]),"NA")
    #}
   
    #for (j in seq_along(TECH_SKILLS)) {
    #jd_df$tech<- ifelse(grepl(TECH_SKILLS[j],jd_df$job_descr[i]),as.character(TECH_SKILLS[j]),"NA")
    #}
...
write.csv(job_attr_df, file ="INDEED_skills_cat.csv", row.names=FALSE)

skills_all<-sqldf("select distinct * from(select id,descr,general as skill from job_attr_df where general <> 'na' 
      union all
      select id,descr,personality as skill from job_attr_df where personality <> 'na' 
      union all
      select id,descr,tech as skill from job_attr_df where tech <> 'na') a")
